...
app.get('/students', async (req,res) => {
    ...
    // const rows = await connection.query("SELECT SLEEP(10)");
    res.send(rows)
})
...
app.get('/students/:id', async (req,res)=>{
    const connection = await dbConn;
    let d = 3;//req.params.id;
    const [rows] = await connection.query('SELECT * from students where ?', [{id: d}])// +req.params.id)
    // const [rows] = await connection.execute('SELECT * from students where id = :id', {id: 3})
    res.json(rows);
    // res.send(rows)
})
...
app.put("/students/:id", async (req, res) => {
    ...
    // const rows = await connection.query("Update students set name = '"+name+"' where id = " + id);
    res.status(201).send(rows)
})
